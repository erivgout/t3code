...
const PROCESS_QUERY_TIMEOUT_MS = 30_000;
...
function assertDescendantPid(
  ...
  if (pid === process.pid) {
    return Effect.fail(
      toProcessDiagnosticsError("Refusing to signal the Kalsec Code server process."),
    );
  }
      ...
      return descendant
        ...
        : Effect.fail(
            toProcessDiagnosticsError(
              `Process ${pid} is not a live descendant of the Kalsec Code server.`,
            ),
          );
    }),
  );
